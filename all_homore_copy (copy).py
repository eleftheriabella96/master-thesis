...
library_name = 'mit-bih-supraventricular-arrhythmia'
countlines = 0
tags = []
...
#with open('RR_differences'+ str(number) +'.txt') as file_RR:
	...
	y1 = heartrate_new[RMSSD_index*window_size:(RMSSD_index+1)*window_size]#window of max RMSSD value
	...
	y2 = heartrate_new[RMSSD_second_index*window_size:(RMSSD_second_index+1)*window_size]#window of max RMSSD value
	...
	y3 = heartrate_new[RMSSD_third_index*window_size:(RMSSD_third_index+1)*window_size]#window of max RMSSD value
	...
	y4 = heartrate_new[RMSSD_fourth_index*window_size:(RMSSD_fourth_index+1)*window_size]#window of max RMSSD value
	...
	y5 = heartrate_new[RMSSD_fifth_index*window_size:(RMSSD_fifth_index+1)*window_size]#window of max RMSSD value
	...
	#y2 = heartrate_new[APEN_index*window_size:(APEN_index+1)*window_size]#window of max ApEn value
	
	#approximate_array.sort()
	#APEN_second_index = approximate_array.index(approximate_array[-2])
	#x2_ = [i for i in range(APEN_second_index*window_size,(APEN_second_index+1)*window_size)]
	#y2_ = heartrate_new[APEN_second_index*window_size:(APEN_second_index+1)*window_size]#window of max RMSSD value	
	...
	#plt.plot(x2, y2, color = 'red', linewidth = 0.5, zorder = 4)
	#plt.plot(x2_, y2_, color = 'yellow', linewidth = 0.5, zorder = 5)
	figurename0 =  library_name + "_" + str(filename_record) + ".jpeg" #"Usr" + str(user)+"0.jpeg"
